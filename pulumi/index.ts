...
const dataLakeBucketOwnership = new aws.s3.BucketOwnershipControls("data_lake_bucket_ownership", {
...
// Bucket versioning
const dataLakeBucketVersioning = new aws.s3.BucketVersioningV2("data_lake_bucket_versioning", {
    bucket: dataLakeBucket.id,
...
const dataLakeBucketEncryption = new aws.s3.BucketServerSideEncryptionConfigurationV2("data_lake_bucket_encryption", {
    bucket: dataLakeBucket.id,
    rules: [{
...
const dataLakeBucketLifecycle = new aws.s3.BucketLifecycleConfigurationV2("data_lake_bucket_lifecycle", {
    bucket: dataLakeBucket.id,
    rules: [{
        id: "transition-to-ia",
        status: "Enabled",
...
// Block public access
const dataLakeBucketPublicAccessBlock = new aws.s3.BucketPublicAccessBlock("data_lake_bucket_public_access_block", {
    bucket: dataLakeBucket.id,
...
const lambdaArtifactsBucketVersioning = new aws.s3.BucketVersioningV2("lambda_artifacts_versioning", {
...
const lambdaArtifactsBucketEncryption = new aws.s3.BucketServerSideEncryptionConfigurationV2("lambda_artifacts_encryption", {
    bucket: lambdaArtifactsBucket.id,
    rules: [{
        applyServerSideEncryptionByDefault: {
            sseAlgorithm: "AES256",
...
const lambdaArtifactsBucketPublicAccessBlock = new aws.s3.BucketPublicAccessBlock("lambda_artifacts_public_access_block", {
    bucket: lambdaArtifactsBucket.id,
    blockPublicAcls: true,
    blockPublicPolicy: true,
    ignorePublicAcls: true,
...
const s3NotificationsTopicPolicy = new aws.sns.TopicPolicy("s3_notifications_policy", {
...
const dataProcessingQueuePolicy = new aws.sqs.QueuePolicy("data_processing_policy", {
...
const lambdaPlaceholder = new archive.File("lambda_placeholder", {
    type: "zip",
...
const dataProcessor = new aws.lambda.Function("data_processor", {
    ...
    timeout: 60,
    memorySize: 256,
    code: lambdaPlaceholder.outputPath.apply(path => new pulumi.asset.FileArchive(path)),
    sourceCodeHash: lambdaPlaceholder.outputBase64sha256,
    environment: {
        variables: {
