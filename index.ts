...
// =============================================================================
const lambdaArtifactsBucket = new aws.s3.Bucket("lambda-artifacts", {
...
const lambdaArtifactsVersioning = new aws.s3.BucketVersioning("lambda-artifacts-versioning", {
...
const lambdaArtifactsEncryption = new aws.s3.BucketServerSideEncryptionConfiguration("lambda-artifacts-encryption", {
    bucket: lambdaArtifactsBucket.id,
    rules: [{
...
const lambdaArtifactsPublicAccessBlock = new aws.s3.BucketPublicAccessBlock("lambda-artifacts-pab", {
    bucket: lambdaArtifactsBucket.id,
...
// =============================================================================
const dataLakeBucket = new aws.s3.Bucket("data-lake-bucket", {
...
const dataLakeBucketOwnership = new aws.s3.BucketOwnershipControls("data-lake-bucket-ownership", {
...
const dataLakeBucketVersioning = new aws.s3.BucketVersioning("data-lake-bucket-versioning", {
    bucket: dataLakeBucket.id,
    versioningConfiguration: {
        status: "Enabled",
    },
...
const dataLakeBucketEncryption = new aws.s3.BucketServerSideEncryptionConfiguration("data-lake-bucket-encryption", {
    bucket: dataLakeBucket.id,
    rules: [{
        applyServerSideEncryptionByDefault: {
...
const dataLakeBucketLifecycle = new aws.s3.BucketLifecycleConfiguration("data-lake-bucket-lifecycle", {
    bucket: dataLakeBucket.id,
    rules: [{
        id: "transition-to-ia",
        status: "Enabled",
...
const dataLakeBucketPublicAccessBlock = new aws.s3.BucketPublicAccessBlock("data-lake-bucket-pab", {
    bucket: dataLakeBucket.id,
    blockPublicAcls: true,
    blockPublicPolicy: true,
    ignorePublicAcls: true,
...
// =============================================================================
const s3NotificationsTopic = new aws.sns.Topic("s3-notifications", {
...
const s3NotificationsTopicPolicy = new aws.sns.TopicPolicy("s3-notifications-policy", {
...
// =============================================================================
const dataProcessingDlq = new aws.sqs.Queue("data-processing-dlq", {
...
const dataProcessingQueue = new aws.sqs.Queue("data-processing", {
...
const dataProcessingQueuePolicy = new aws.sqs.QueuePolicy("data-processing-policy", {
...
const lambdaS3AccessPolicy = new aws.iam.RolePolicy("lambda-s3-access", {
...
// =============================================================================
const lambdaArchive = new archive.File("lambda-placeholder", {
    type: "zip",
...
const dataProcessor = new aws.lambda.Function("data-processor", {
    ...
    timeout: 60,
    memorySize: 256,
    code: lambdaArchive.outputPath.apply(path => new pulumi.asset.FileArchive(path)),
...
const sqsTrigger = new aws.lambda.EventSourceMapping("sqs-trigger", {
...
const lambdaLogGroup = new aws.cloudwatch.LogGroup("lambda-logs", {
...
// =============================================================================
const fileMetadataTable = new aws.dynamodb.Table("file-metadata", {
